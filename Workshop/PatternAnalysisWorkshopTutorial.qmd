...
With all this work, we may want to save these maps for analysis outside of this script.
...
Examine a small area of this to see the detailed pattern.
...
Here we see a very detailed pattern. This is part of the Cornell University properties, surveyed by @Cline.Bloom1965. This area will be highlighted in the moving-window autocorrelation (@sec-mwla).
...
Task: Convert the `terra::SpatRaster` objects to `raster::raster` and then to `sf:sf` objects in order to compute variograms. The `gstat::variogram` method must be applied to an object of class `sp` or `sf`, not directly to a `terra::SpatRaster`.
...
Here is a function to make an odd-sized square window with weights from the variogram model, scaled to the resolution.
...
Q: Are the patterns of local autocorrelation the same for both maps? Which areas (if any) are different?

Q: Which map has larger differences?

Q: How do the difference change as the window size increases?
...
The GLCM approximates the joint probability distribution of the levels of two pixels separated bythe specified shift(s), that is, how likely it is that these two levels occur together in the window. We would like to avoid zero probabilities. If there are too many levels, many pairs of  will not occur. So we should pick a number of levels for quantization which avoids this.

For the SoilGrid map most values of the soil property are different, in fact in this example there are `r dim(unique(sg.utm))[1]` different values, not many fewer than the pixel count, `r prod(dim(sg.utm))`. The same is true for gNATSGO, due to the reprojection by bilinear interpolation. So quantization is required. 
...
The lowest quantiles (lowest pH values) are on the hills to the east of the map, the highest quantiles (highest pH values) in the stream valleys to the west. It is difficult to see just from this map if the GLCM will have too many zeroes, or ifa finer quantization could be supported.

### Constructing a GLCM

This section shows how a GLCM is constructed. We take a simple example of a 16-class quantization and a 5x5 window near the middle of the map, and a one-cell rightward shift.
...
The original matrix is 5 x 5 cells; the GLCM is 8 x 8 levels.

We see in this window that almost all  elements are on the diagonal, i.e., no change in level based on the 16-level GLCM. This is thus a fairly homogeneous window when shifted one cell to the right. The values in this window all fell in the three lowest pH classes: `r l.16[1]` to `r l.16[3]`.
