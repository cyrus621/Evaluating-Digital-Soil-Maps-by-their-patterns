...
where $y_i$ is the value of the variable in the $i$th of $n$ neighbouring grid cells, $\bar{y}$ is the global mean of the variable, $w_{ij}$ is the spatial **weight** of the link between the target cell $i$ and its neighbour cell $j$. The expected value of Moran's I is $-1/(n-1)$ if the pattern of the response variable is random, i.e., no spatial correlation. So for a $5 \times 5$ neighbourhood the expected value if random is $-1/24 = -0.041\bar{6} \approx 0$.
...
We determine the weights matrix for Moran's I from the global variogram analysis and the grid cell size. Weights are the one minus the semivariance at each cell distance, so that the centre pixel receives the maximum weight. 
...
These are both very far from the random value $-0.041\bar{6}$. Both maps show hot spots with much larger local autocorrelation than the map average. Some areas have almost none or even more dispersed than random (negative values).
...
We see in this window many diagonal elements in the GLCM based on 16 levels, thus a fairly homogeneous window when shifted one cell to the right. The values in this window all fell in the three lowest pH classes: `r l.16[1]` to `r l.16[3]`.
